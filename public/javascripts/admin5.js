...
new Vue({
    ...
    mounted(){
        ...
        this.fetchUsersForBranch(branchId);
    },
    methods: {
        fetchUsersForBranch(branchId){
        fetch(`/admin/getUsersNotInBranch?branchId=${branchId}`)
          .then(response => response.json())
          .then(data => {
            this.branch.branchId = data.branchId;
            this.users = data.users;
          })
          .catch(error => console.error('Error:', error));
        },
        AddThisUserToBranch(a){
            const addData = {
            ...
            xhttp.onreadystatechange = () => {
                if (xhttp.readyState === 4 ) {
                  if (xhttp.status === 200) {
                    this.users = this.users.filter(user => user.id !== a);
                  }
                } else if (xhttp.readyState === 4 && xhttp.status === 401) {
                    alert("Unsuccessful");
                }
              };
            xhttp.open('POST', '/admin/adminAddUserToBranch', true);
